# -*- coding: utf-8 -*-
"""
全自动Tau PET影像分类与2.5D Vision Transformer训练流水线 (V2.0 - 路径修正与功能补全版)
=========================================================================================
本脚本是一个“一键式”的完整解决方案，用于在Tau PET数据上训练和评估2.5D ViT模型。
此版本进行了以下关键修改：
- **动态路径管理**: 所有数据和模型保存路径都基于脚本的当前位置动态生成，实现了代码的可移植性。
- **增强的健壮性**: 自动创建模型保存目录，并在评估前检查文件是否存在。
- **注意力可视化**: 重新加入了最终的注意力图谱可视化功能，用于解释模型决策。
...
前置要求:
- 项目结构符合规范 (例如，此脚本、dataset, models等目录都在项目根目录下)。
...
# 【已修正】获取项目根目录，以便构建所有绝对路径
# 假设此脚本位于项目文件夹的某个子目录中，例如 <PROJECT_ROOT>/src/
# .parents[n] 中的 'n' 需要根据脚本的实际深度进行调整
try:
    PROJECT_ROOT = Path(__file__).resolve().parents[2]
except NameError:
    # 在交互式环境（如Jupyter）中，__file__未定义，使用当前工作目录
...
def create_adni_data_list():
    ...
    pet_csv_path = CSV_DIR / 'All_Subjects_UCBERKELEY_TAUPVC_6MM_10Aug2025.csv'
    cdr_csv_path = CSV_DIR / 'All_Subjects_CDR_10Aug2025.csv'
    npy_base_dir = NPY_ROOT / "tau_pet"
    # --- 配置区结束 ---
...
def train_model(train_df, val_df):
    ...
    UNIMODAL_MODELS_DIR = MODELS_ROOT / "tau_pet_unimodal"
    BEST_MODEL_PATH = UNIMODAL_MODELS_DIR / 'tau_adni_pretrained_vit_tiny_best.pth'
    # --- 配置区结束 ---
...
def final_evaluation(model_path, val_loader, device, model_name):
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(val_loader, desc="正在验证集上评估"):
            if inputs.numel() == 0: continue
            inputs, labels_cpu = inputs.to(device), labels
            with autocast(device_type=device):
                outputs = model(inputs)
    ...
    try:
        ...
        # attn_weights 的形状是 (Batch, Num_Slices, Num_Slices)
        for i in range(min(4, inputs.shape[0])):
            # 【已修正】使用正确的逻辑为每个样本计算切片重要性
            # 对一个样本的注意力图谱在dim=0上取平均，得到每个切片作为“输出”时，对所有“输入”切片的平均关注度
            slice_importance = attn_weights[i].mean(dim=0).cpu().numpy()
