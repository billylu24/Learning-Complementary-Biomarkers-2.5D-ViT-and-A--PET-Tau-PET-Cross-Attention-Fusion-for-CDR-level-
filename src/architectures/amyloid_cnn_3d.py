# -*- coding: utf-8 -*-
"""
全自动PET影像分类与3D-CNN训练流水线 (ADNI增强版)
============================================================
本脚本是一个“一键式”的完整解决方案，整合了从数据清单生成到模型训练的所有步骤。
V6更新: 将图片输出替换为包含AUC、混淆矩阵等指标的详细文字评估报告。

它会自动执行以下流程:
1.  **数据清单生成 (步骤一)**:
    - 加载ADNI官方的PET影像信息CSV和临床CDR评分CSV。
...
def create_adni_data_list():
    ...
    # 3. 定义具体文件路径
    pet_csv_path = CSV_DIR / 'All_Subjects_UCBERKELEY_AMY_6MM_10Aug2025.csv'
...
def train_3d_cnn_model(train_df, val_df):
    ...
    BEST_MODEL_NAME = 'adni_3d_cnn_best.pth'
    # --- 配置区结束 ---
    ...
    class Pet3DCNN(nn.Module):
        def __init__(self):
            ...
            self.model = nn.Sequential(
                ...
                nn.Dropout(0.4), nn.Linear(256, 1), nn.Sigmoid()
            )
...
def final_evaluation(model_path, val_loader, device):
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(val_loader, desc="正在验证集上评估"):
            ...
            all_preds_probs.extend(outputs.cpu().numpy().flatten())
